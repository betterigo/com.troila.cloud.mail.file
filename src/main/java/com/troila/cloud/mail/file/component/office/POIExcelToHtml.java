...
public class POIExcelToHtml {
	...
	public static String excelToHtml(String SourcePath,String targetPath , boolean isWithStyle) {
		...
		try {
			...
			if (wb instanceof XSSFWorkbook) {
				...
				FileUtils.writeByteArrayToFile(new File(targetPath), htmlExcel.getBytes("utf-8"));
			} else if (wb instanceof HSSFWorkbook) {
				HSSFWorkbook hWb = (HSSFWorkbook) wb;
				htmlExcel = POIExcelToHtml.getExcelInfo(hWb, isWithStyle);
				FileUtils.writeByteArrayToFile(new File(targetPath), htmlExcel.getBytes("gb2312"));
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public static void main(String[] args) {
		excelToHtml("D://aaa.xls","D://aaahtml.html",true);
	}
}